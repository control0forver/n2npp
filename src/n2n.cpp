...
N2N_THREAD_RETURN_DATATYPE resolve_thread(N2N_THREAD_PARAMETER_DATATYPE p) {
    ...
    n2n_resolve_parameter *param = (n2n_resolve_parameter*)p;
...
int resolve_create_thread (n2n_resolve_parameter **param, struct peer_info *sn_list) {
    ...
    *param = (n2n_resolve_parameter*)calloc(1, sizeof(n2n_resolve_parameter));
    if(*param) {
        HASH_ITER(hh, sn_list, sn, tmp_sn) {
            ...
            if(sn->ip_addr) {
                ...
                if(entry) {
                    ...
                    memcpy(&(entry->sock), &(sn->sock), sizeof(n2n_sock));
...
uint8_t resolve_check (n2n_resolve_parameter *param, uint8_t requires_resolution, time_t now) {
    ...
    if((now - param->last_checked > param->check_interval) || (requires_resolution)) {
        ...
        if(pthread_mutex_trylock(&param->access) == 0) {
            ...
            if(param->changed) {
                ...
                HASH_ITER(hh, param->list, entry, tmp_entry) {
                    memcpy(entry->org_sock, &entry->sock, sizeof(n2n_sock));
    ...
    if(peer == NULL) { /* zero MAC, search by socket */
        ...
        if((peer == NULL) && (*skip_add == SN_ADD)) {
            ...
            if(peer) {
                ...
                peer->purgeable = true;
                memcpy(&(peer->sock), sock, sizeof(n2n_sock));
...
uint64_t time_stamp (void) {
    ...
    counter &= NEGATIVE_MASK(hi_unchanged);
