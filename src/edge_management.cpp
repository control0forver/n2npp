...
#include "n2n.h"           // for n2n_edge, peer_info, getTraceLevel, N2N_...
...
#include "n2n_typedefs.h"  // for n2n_edge, peer_info, n2n_edge_conf
#include "sn_selection.h"  // for sn_selection_criterion_str, selection_crit...
#include "strbuf.h"        // for strbuf, STRBUF_INIT
...
size_t event_debug (strbuf *buf, char *tag, int data0, void *data1) {
    traceEvent(TRACE_DEBUG, "Unexpected call to event_debug");
    return 0;
}

size_t event_test (strbuf *buf, char *tag, int data0, void *data1) {
    size_t msg_len = gen_json_1str(buf, tag, "event", "test", (char *)data1);
    return msg_len;
}

size_t event_peer (strbuf *buf, char *tag, int data0, void *data1) {
    int action = data0;
...
static void mgmt_communities (mgmt_req_t *req, strbuf *buf) {
...
static void mgmt_supernodes (mgmt_req_t *req, strbuf *buf) {
    size_t msg_len;
...
static void mgmt_edges_row (mgmt_req_t *req, strbuf *buf, struct peer_info *peer, const char *mode) {
    size_t msg_len;
    macstr_t mac_buf;
    n2n_sock_str_t sockbuf;
...
static void mgmt_edges (mgmt_req_t *req, strbuf *buf) {
    struct peer_info *peer, *tmpPeer;
...
static void mgmt_edge_info (mgmt_req_t *req, strbuf *buf) {
    size_t msg_len;
    macstr_t mac_buf;
...
static void mgmt_timestamps (mgmt_req_t *req, strbuf *buf) {
    size_t msg_len;
...
static void mgmt_packetstats (mgmt_req_t *req, strbuf *buf) {
    size_t msg_len;
...
static void mgmt_post_test (mgmt_req_t *req, strbuf *buf) {
...
static void mgmt_help (mgmt_req_t *req, strbuf *buf);
static void mgmt_help_events (mgmt_req_t *req, strbuf *buf);

static const mgmt_handler_t mgmt_handlers[] = {
    { .flags = FLAG_WROK, .cmd = "reload_communities", .help = "Reserved for supernode",          .func = mgmt_unimplemented},

    { .flags = FLAG_WROK, .cmd = "stop",               .help = "Gracefully exit edge",            .func = mgmt_stop},
    { .flags = FLAG_WROK, .cmd = "verbose",            .help = "Manage verbosity level",          .func = mgmt_verbose},
    ...
    { .flags = FLAG_WROK, .cmd = "help",               .help = "Show JSON commands",              .func = mgmt_help},
...
static mgmt_req_t mgmt_event_subscribers[] = {
    { .eee = NULL, .type = N2N_MGMT_UNKNOWN, .tag = "\0" }, // N2N_EVENT_DEBUG = 0
    { .eee = NULL, .type = N2N_MGMT_UNKNOWN, .tag = "\0" }, // N2N_EVENT_TEST  = 1
    { .eee = NULL, .type = N2N_MGMT_UNKNOWN, .tag = "\0" }, // N2N_EVENT_PEER  = 2
};

/* Map topic number to function */
// TODO: want this to be const
static mgmt_event_handler_t *mgmt_events[] = {
    event_debug,  // N2N_EVENT_DEBUG = 0
    event_test,   // N2N_EVENT_TEST  = 1
    event_peer,   // N2N_EVENT_PEER  = 2
};

/* Allow help and subscriptions to use topic name */
static const mgmt_events_t mgmt_event_names[] = {
    { .topic = N2N_EVENT_DEBUG, .cmd = "debug", .help = "All events - for event debugging"},
    { .topic = N2N_EVENT_TEST,  .cmd = "test",  .help = "Used only by post.test"},
    { .topic = N2N_EVENT_PEER,  .cmd = "peer",  .help = "Changes to peer list"},
};
