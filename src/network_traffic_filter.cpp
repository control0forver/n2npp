...
#include "n2n.h"                     // for filter_rule, filter_rule_pair_...
...
/* for [-Wmissing-declarations] */
const char* get_filter_packet_info_log_string (packet_address_proto_info* info);
...
/* for [-Wmissing-declarations] */
void collect_packet_info (packet_address_proto_info* out_info, unsigned char *buffer, int size);

void collect_packet_info (packet_address_proto_info* out_info, unsigned char *buffer, int size) {

    ether_hdr *hdr_ether = (ether_hdr*)buffer;
    ...
    memset(out_info, 0, sizeof(packet_address_proto_info));

    switch(ether_type) {
        case 0x0800: {
            buffer += sizeof(ether_hdr);
            size -= sizeof(ether_hdr);
            if(size <= 0) {
                return;
            }
...
/* for [-Wmissing-declarations] */
const char* get_filter_rule_info_log_string (filter_rule* rule);
...
/* for [-Wmissing-declarations] */
uint8_t march_rule_and_cache_key (filter_rule_key *rule_key, packet_address_proto_info *pkt_addr_info);
...
/* for [-Wmissing-declarations] */
filter_rule* get_filter_rule (filter_rule **rules, packet_address_proto_info *pkt_addr_info);

filter_rule* get_filter_rule (filter_rule **rules, packet_address_proto_info *pkt_addr_info) {

    filter_rule *item = 0, *tmp = 0, *marched_rule = 0;
    int march_score = 0;
...
/* for [-Wmissing-declarations] */
void update_and_clear_cache_if_need (network_traffic_filter *filter);

void update_and_clear_cache_if_need (network_traffic_filter *filter) {

    if(++(filter->work_count_scene_last_clear) > CLEAR_CACHE_EVERY_X_COUNT) {
        filter_rule_pair_cache *item = NULL, *tmp = NULL;
...
/* for [-Wmissing-declarations] */
filter_rule_pair_cache* get_or_create_filter_rule_cache (network_traffic_filter *filter, packet_address_proto_info *pkt_addr_info);

filter_rule_pair_cache* get_or_create_filter_rule_cache (network_traffic_filter *filter, packet_address_proto_info *pkt_addr_info) {

    filter_rule_pair_cache* rule_cache_find_result = 0;
    HASH_FIND(hh, filter->connections_rule_cache, pkt_addr_info, sizeof(packet_address_proto_info), rule_cache_find_result);
    if(!rule_cache_find_result) {
        filter_rule* rule = get_filter_rule(&filter->rules, pkt_addr_info);
        if(!rule) {
            return NULL;
        }

        rule_cache_find_result = (filter_rule_pair_cache*)malloc(sizeof(filter_rule_pair_cache));
        memset(rule_cache_find_result, 0, sizeof(filter_rule_pair_cache));
        ...
        HASH_ADD(hh, filter->connections_rule_cache, key, sizeof(packet_address_proto_info), rule_cache_find_result);
    }
...
/* for [-Wmissing-declarations] */
n2n_verdict filter_packet_from_peer (network_traffic_filter *filter, n2n_edge *eee, const n2n_sock *peer, uint8_t *payload, uint16_t payload_size);

n2n_verdict filter_packet_from_peer (network_traffic_filter *filter, n2n_edge *eee, const n2n_sock *peer, uint8_t *payload, uint16_t payload_size) {

    filter_rule_pair_cache *cur_pkt_rule = 0;
    packet_address_proto_info pkt_info;
...
/* for [-Wmissing-declarations] */
network_traffic_filter *create_network_traffic_filter ();

network_traffic_filter *create_network_traffic_filter () {

    network_traffic_filter *filter = (network_traffic_filter*)malloc(sizeof(network_traffic_filter));

    memset(filter, 0, sizeof(network_traffic_filter));
...
/* for [-Wmissing-declarations] */
void destroy_network_traffic_filter (network_traffic_filter *filter);

void destroy_network_traffic_filter (network_traffic_filter *filter) {

    filter_rule *el = 0, *tmp = 0;
    filter_rule_pair_cache* el1 = 0, * tmp1 = 0;
...
/* for [-Wmissing-declarations] */
void network_traffic_filter_add_rule (network_traffic_filter* filter, filter_rule* rules);

void network_traffic_filter_add_rule (network_traffic_filter* filter, filter_rule* rules) {

    filter_rule *item = NULL, *tmp = NULL;

    HASH_ITER(hh, rules, item, tmp) {
        filter_rule *new_rule = (filter_rule*)malloc(sizeof(filter_rule));
        memcpy(new_rule, item, sizeof(filter_rule));
        HASH_ADD(hh, filter->rules, key, sizeof(filter_rule_key), new_rule);
...
/* for [-Wmissing-declarations] */
void process_traffic_filter_proto (const char* begin, const char* next_pos_of_last_char, filter_rule *rule_struct);
...
/* for [-Wmissing-declarations] */
uint8_t process_traffic_filter_rule_str (const char *rule_str, filter_rule *rule_struct);
