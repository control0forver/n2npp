...
int initWin32() {
  ...
  if( err != 0 ) {
    ...
    /* WinSock DLL.                                  */
    printf("FATAL ERROR: unable to initialise Winsock 2.x. (WSAStartup: %d, WSAGetLastError: %d)", err, WSAGetLastError());
    // exit(EXIT_FAILURE);
    return 1;
  }
...
static HANDLE open_tap_device(const char *adapterid) {
  char tapname[1024];
  _snprintf(tapname, sizeof(tapname), USERMODEDEVICEDIR "%s" TAP_WIN_SUFFIX, adapterid);
...
static int iterate_win_network_adapters(
  ...
  for (i = 0; ; i++) {
    ...
    if(adapter.handle != INVALID_HANDLE_VALUE) {
      ...
      if(!callback(&adapter, (tuntap_dev*)userdata))
          break;
      else
...
void win_print_available_adapters() {
  auto proc = iterate_win_network_adapters(print_adapter_callback, NULL);
  if (proc != 0)
  {
    printf("Failed to iterate adapters. (%d)", proc);
  }
}
...
static int lookup_adapter_info_reg(const char *target_adapter, char *regpath, size_t regpath_size) {
  ...
  for(i = 0; ; i++) {
    ...
    _snprintf(regpath, regpath_size, "%s\\%s", ADAPTER_KEY, index);
    if(RegOpenKeyEx(HKEY_LOCAL_MACHINE, (LPCSTR)regpath, 0, KEY_READ, &key2))
      continue;
...
static int set_interface_mac(struct tuntap_dev *device, const char *mac_str) {
  char cmd[256];
  char mac_buf[18];
...
int open_wintap(struct tuntap_dev *device,
  ...
  int rc_get_adapter = iterate_win_network_adapters(choose_adapter_callback, device);
  if(rc_get_adapter != 0) {
    printf("An error occurred while retrieving the list of Windows tap devices (%d)\n", rc_get_adapter);
    return -1;
  }
  ...
  buffer = (IP_ADAPTER_INFO*)malloc(buffer_len);
  ...
  if(device_mac && device_mac[0])
    if (set_interface_mac(device, device_mac) != 0) {
      printf("Cannot set MAC address for Windows tap %s (%s)\n",
  ...
  if(metric) { /* try to change only if a value has been given, otherwise leave with default or as set before */
    ...
    Row = (MIB_IPINTERFACE_ROW*)calloc(1, sizeof(MIB_IPINTERFACE_ROW));
