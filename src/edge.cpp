...
#include "n2n.h"                     // for n2n_edge_conf, n2n_edge, fil...
...
#ifdef _M_CEE
#include "n2n_library.hpp"             // for complete library exports
#endif
...
void send_register_super (n2n_edge *eee);
void send_query_peer (n2n_edge *eee, const n2n_mac_t dst_mac);
int supernode_connect (n2n_edge *eee);
int supernode_disconnect (n2n_edge *eee);
int fetch_and_eventually_process_data (n2n_edge *eee, SOCKET sock,
                                       uint8_t *pktbuf, uint16_t *expected, uint16_t *position,
                                       time_t now);
uint8_t resolve_check (n2n_resolve_parameter *param, uint8_t resolution_request, time_t now);
...
static void help (int level, bool legacy_mode) {
...
static void show_help (int level) {
    help(level, false);
}

/* *************************************************** */

static int setPayloadCompression (n2n_edge_conf *conf, int compression) {
...
static int setPayloadEncryption (n2n_edge_conf *conf, int cipher) {
    ...
    switch (cipher) {
        ...
        default: {
            ...
            traceEvent(TRACE_NORMAL, "the %s cipher given by -A_ option is not supported in this version", transop_str(static_cast<n2n_transform>(cipher)));
            return 1;
        }
    }
...
static int setOption (int optkey, char *optargument, n2n_tuntap_priv_config *ec, n2n_edge_conf *conf) {
    ...
    switch(optkey) {
        ...
        case 'A': {
            ...
            if (setPayloadEncryption(conf, cipher) != 0)
                return 1;
            break;
        }
        ...
        case 'z': {
            ...
            if (setPayloadCompression(conf, compression) != 0)
                return 1; // to make user aware (:
            break;
        }
        ...
        case 'J': /* password for user-password authentication */ {
            if(!conf->shared_secret) /* we could already have it from environment variable, see edge_init_conf_defaults() */
                conf->shared_secret = (n2n_private_public_key_t*)calloc(1, sizeof(n2n_private_public_key_t));
        ...
        case 'R': /* network traffic filter */ {
            auto new_rule = (filter_rule*)malloc(sizeof(filter_rule));
            memset(new_rule, 0, sizeof(filter_rule));

            if(process_traffic_filter_rule_str(optargument, new_rule)) {
                HASH_ADD(hh, conf->network_traffic_filter_rules, key, sizeof(filter_rule_key), new_rule);
            } else {
                free(new_rule);
...
static int loadFromCLI (int argc, char *argv[], n2n_edge_conf *conf, n2n_tuntap_priv_config *ec) {
                            ...
                            long_options, NULL)) != '?') 
    {
        ...
        int ex = setOption(c, optarg, ec, conf);
        if (ex != 0)
        {
            show_help(ex);
            return 1;
...
static int loadFromFile (const char *path, n2n_edge_conf *conf, n2n_tuntap_priv_config *ec) {
    ...
    while((line = fgets(buffer, sizeof(buffer), fd)) != NULL) {
        ...
        auto __not_used = loadFromCLI(line_vec[2] ? 3 : 2, line_vec, conf, ec);
    }
...
#ifdef _WIN32
BOOL WINAPI term_handler(DWORD fdwCtrlType)
#else
...
int main (int argc, char* argv[]) {
    ...
    n2n_edge *eee;                /* single instance for this program */
    n2n_edge_conf conf;           /* generic N2N edge config */
    n2n_tuntap_priv_config ec;    /* config used for standalone program execution */
    ...
    peer_info *scan, *scan_tmp;   /*            supernode iteration */
    ...
    while(runlevel < 5) {
        ...
        if(runlevel > 1) {
            if(eee->sn_pong) {
                ...
                if(eee->curr_sn->hh.next) {
                    sn_selection_sort((peer_info**)&(eee->curr_sn->hh.next));
        ...
        if(runlevel == 3) { /* REGISTER_SUPER to get auto ip address from a sn has been sent */
            if(!eee->sn_wait) { /* TUNTAP IP address received */
                ...
                if(eee->curr_sn->hh.next)
                    eee->curr_sn = (peer_info*)eee->curr_sn->hh.next;
                else
                    eee->curr_sn = eee->conf.supernodes;
                supernode_connect(eee);
                runlevel--;
...
#else
#ifndef __APPLE__
    traceEvent(TRACE_WARNING, "n2n has not been compiled with libcap-dev on Linux/Unix; some commands may fail");
#endif
#endif /* HAVE_LIBCAP */
...
#include <iostream>
static int OOPMAIN(int argc, char* argv[])
{
    auto edge = n2n::edge::Edge();
    edge.LoadOptionsCLI(argc, argv);

    [[maybe_unused]] auto _ = edge.Start();

    while (edge.IsRunning()) {
        using namespace std;

        cout << "enter 'stop' to stop: ";
        string input;
        getline(cin, input);
        if (input == "stop")
            break;
    ...
    while (edge.IsRunning()) {
        printf("Edge is pending stop, please wait...\n");
        std::this_thread::sleep_for(std::chrono::milliseconds(2500));

        // ...
    }
    return 12345; // this value should not be used
}

namespace n2n::edge
{
    int Edge::main(int argc, char* argv[]) { return ::main(argc, argv); }
    ...
    Edge::Edge() {
        ...
        // n2n_edge_conf
        edge_init_conf_defaults(&this->Config);

        // n2n_tuntap_priv_config
        this->ec =
        {
            .mtu = DEFAULT_MTU,
            .daemon = 1, /* By default run in daemon mode. */
        };
        ...
        if (((pw = getpwnam("n2n")) != NULL) ||
            ((pw = getpwnam("nobody")) != NULL)) {
            this->ec.userid = pw->pw_uid;
            this->ec.groupid = pw->pw_gid;
        }
#endif
        
        // Tuntap device set
#ifdef _WIN32
        this->ec.tuntap_dev_name[0] = '\0'; // default for picking the first valid adapter if the argument 'd' is not specified (check choose_adapter_callback in 'wintap.cpp')
        this->ec.metric = 0; // 网卡跃点数，对应参数 "-x"，Windows Only
#else
        snprintf(this->ec.tuntap_dev_name, sizeof(this->ec.tuntap_dev_name), N2N_EDGE_DEFAULT_DEV_NAME); // "edge0"
#endif

        // Netmask
        snprintf(this->ec.netmask, sizeof(this->ec.netmask), N2N_EDGE_DEFAULT_NETMASK);
    }

    int Edge::LoadOptions(const std::vector<std::tstring>& options) {
        int _args_count = options.size() + 1;

        std::vector<chara*> _args{};
        _args.emplace_back(UseAStr(__FUNCTION__)); // argv[0]
        for (const auto& o : options)
            _args.emplace_back(UseAStr(o.data()));
        _args.emplace_back(nullptr); // argv end

        if (loadFromCLI(_args_count, _args.data(), &this->Config, &this->ec) == 0)
        {
            OnOptionsLoaded();
            return 0;
        }
    ...
    int Edge::LoadOptionsCLI(int argc, tchar* const argv[]) {
        std::vector<char*> _args{};
        for (int io = 0; io < argc; io++)
            _args.emplace_back(UseAStr(argv[io]));
        _args.emplace_back(nullptr); // argv end

        if (loadFromCLI(argc, _args.data(), &this->Config, &this->ec) == 0)
        {
            OnOptionsLoaded();
            return 0;
        }
    ...
    int Edge::LoadOptionsFile(const std::tstring& path) {
        if (loadFromFile(path.c_str(), &this->Config, &this->ec) == 0)
        {
            OnOptionsLoaded();
            return 0;
        }
    ...
    int Edge::Run() {
        ...
        macstr_t mac_buf;             /*            output mac address */
        fd_set socket_mask;           /*            for supernode answer */
        timeval wait_time;            /*            timeout for sn answer */
        peer_info* scan, * scan_tmp;  /*            supernode iteration */
        #ifdef HAVE_LIBCAP
        cap_t caps;
        #endif
        ...
        #endif /* ndef HAVE_LIBCAP */
        #endif /* ndef _WIN32 */

        // --- additional crypto setup; REVISIT: move to edge_init()?
        // payload
        if (this->Config.transop_id == N2N_TRANSFORM_ID_NULL) {
            if (this->Config.encrypt_key) {
                // make sure that AES is default cipher if key only (and no cipher) is specified
                traceEvent(TRACE_WARNING, "switching to AES as key was provided");
                this->Config.transop_id = N2N_TRANSFORM_ID_AES;
            }
        }
        // user auth
        if (this->Config.shared_secret /* containing private key only so far*/) {
            // if user-password auth and no federation public key provided, use default
            if (!this->Config.federation_public_key) {
                this->Config.federation_public_key = (n2n_private_public_key_t*)calloc(1, sizeof(n2n_private_public_key_t));
                if (this->Config.federation_public_key) {
                    traceEvent(TRACE_WARNING, "using default federation public key; FOR TESTING ONLY, usage of a custom federation name and key (-P) is highly recommended!");
                    generate_private_key(*(this->Config.federation_public_key), &FEDERATION_NAME[1]);
                    generate_public_key(*(this->Config.federation_public_key), *(this->Config.federation_public_key));
                }
            }
            // calculate public key and shared secret
            if (this->Config.federation_public_key) {
                traceEvent(TRACE_NORMAL, "using username and password for edge authentication");
                bind_private_key_to_username(*(this->Config.shared_secret), (char*)this->Config.dev_desc);
                this->Config.public_key = (n2n_private_public_key_t*)calloc(1, sizeof(n2n_private_public_key_t));
                if (this->Config.public_key)
                    generate_public_key(*this->Config.public_key, *(this->Config.shared_secret));
                generate_shared_secret(*(this->Config.shared_secret), *(this->Config.shared_secret), *(this->Config.federation_public_key));
                // prepare (first 128 bit) for use as key
                this->Config.shared_secret_ctx = (he_context_t*)calloc(1, sizeof(speck_context_t));
                speck_init((speck_context_t**)&(this->Config.shared_secret_ctx), *(this->Config.shared_secret), 128);
            }
            // force header encryption
            if (this->Config.header_encryption != HEADER_ENCRYPTION_ENABLED) {
                traceEvent(TRACE_NORMAL, "enabling header encryption for edge authentication");
                this->Config.header_encryption = HEADER_ENCRYPTION_ENABLED;
            }
        }

        if (edge_verify_conf(&this->Config) != 0)
            show_help(1); /* short help */
        ...
        traceEvent(TRACE_NORMAL, "using compression: %s.", compression_str(this->Config.compression));
        traceEvent(TRACE_NORMAL, "using %s cipher.", transop_str(this->Config.transop_id));
        ...
        if (this->Config.encrypt_key && !strcmp((char*)this->Config.community_name, this->Config.encrypt_key))
            traceEvent(TRACE_WARNING, "community and encryption key must differ, otherwise security will be compromised");

        int _edge_init_rc = 0;
        if ((this->Data = edge_init(&this->Config, &_edge_init_rc)) == NULL) {
            traceEvent(TRACE_ERROR, "failed in edge_init (%d)", _edge_init_rc);
            return 1;
        }

        memcpy(&(this->Data->tuntap_priv_conf), &ec, sizeof(ec));

        if ((0 == strcmp("static", this->Data->tuntap_priv_conf.ip_mode)) ||
            ((this->Data->tuntap_priv_conf.ip_mode[0] == '\0') && (this->Data->tuntap_priv_conf.ip_addr[0] != '\0'))) {
            traceEvent(TRACE_NORMAL, "use manually set IP address");
            this->Data->conf.tuntap_ip_mode = TUNTAP_IP_MODE_STATIC;
        }
        else if (0 == strcmp("dhcp", this->Data->tuntap_priv_conf.ip_mode)) {
            traceEvent(TRACE_NORMAL, "obtain IP from other edge DHCP services");
            this->Data->conf.tuntap_ip_mode = TUNTAP_IP_MODE_DHCP;
        }
        else {
            traceEvent(TRACE_NORMAL, "automatically assign IP address by supernode");
            this->Data->conf.tuntap_ip_mode = TUNTAP_IP_MODE_SN_ASSIGN;
        }
        ...
        // find at least one supernode alive to faster establish connection
        // exceptions:
        if ((HASH_COUNT(this->Data->conf.supernodes) <= 1) || (this->Data->conf.connect_tcp) || (this->Data->conf.shared_secret)) {
            // skip the initial supernode ping
            traceEvent(TRACE_DEBUG, "skip PING to supernode");
            runlevel = 2;
        }

        this->Data->last_sup = 0; /* if it wasn't zero yet */
        this->Data->curr_sn = this->Data->conf.supernodes;
        supernode_connect(this->Data);
        while (runlevel < 5) {
            ...
            if (runlevel == 0) { /* PING to all known supernodes */
                ...
                this->Data->sn_pong = 0;
                // (re-)initialize the number of max concurrent pings (decreases by calling send_query_peer)
                this->Data->conf.number_max_sn_pings = NUMBER_SN_PINGS_INITIAL;
                send_query_peer(this->Data, null_mac);
                traceEvent(TRACE_NORMAL, "send PING to supernodes");
                runlevel++;
            }

            if (runlevel == 1) { /* PING has been sent to all known supernodes */
                if (this->Data->sn_pong) {
                    // first answer
                    this->Data->sn_pong = 0;
                    sn_selection_sort(&(this->Data->conf.supernodes));
                    this->Data->curr_sn = this->Data->conf.supernodes;
                    supernode_connect(this->Data);
                    traceEvent(TRACE_NORMAL, "received first PONG from supernode [%s]", this->Data->curr_sn->ip_addr);
                    runlevel++;
                }
                else if (last_action <= (now - BOOTSTRAP_TIMEOUT)) {
                    // timeout
            ...
            if (runlevel > 1) {
                if (this->Data->sn_pong) {
                    this->Data->sn_pong = 0;
                    if (this->Data->curr_sn->hh.next) {
                        sn_selection_sort((peer_info**)&(this->Data->curr_sn->hh.next));
                        traceEvent(TRACE_DEBUG, "received additional PONG from supernode");
                        // here, it is hard to detemine from which one, so no details to output
                    }
                }
            ...
            if (runlevel == 2) { /* send REGISTER_SUPER to get auto ip address from a supernode */
                if (this->Data->conf.tuntap_ip_mode == TUNTAP_IP_MODE_SN_ASSIGN) {
                    last_action = now;
                    this->Data->sn_wait = 1;
                    send_register_super(this->Data);
                    runlevel++;
                    traceEvent(TRACE_NORMAL, "send REGISTER_SUPER to supernode [%s] asking for IP address",
                        this->Data->curr_sn->ip_addr);
                }
                else {
                    runlevel += 2; /* skip waiting for TUNTAP IP address */
                    traceEvent(TRACE_DEBUG, "skip auto IP address asignment");
            ...
            if (runlevel == 3) { /* REGISTER_SUPER to get auto ip address from a sn has been sent */
                if (!this->Data->sn_wait) { /* TUNTAP IP address received */
                    runlevel++;
                    traceEvent(TRACE_NORMAL, "received REGISTER_SUPER_ACK from supernode for IP address asignment");
                    // it should be from curr_sn, but we can't determine definitely here, so no details to output
                }
                    ...
                    // timeout, so try next supernode
                    if (this->Data->curr_sn->hh.next)
                        this->Data->curr_sn = (peer_info*)this->Data->curr_sn->hh.next;
                    else
                        this->Data->curr_sn = this->Data->conf.supernodes;
                    supernode_connect(this->Data);
                    runlevel--;
            ...
            if (runlevel == 4) { /* configure the TUNTAP device, including routes */
                if (tuntap_open(&this->TunDevice, this->Data->tuntap_priv_conf.tuntap_dev_name, this->Data->tuntap_priv_conf.ip_mode,
                                this->Data->tuntap_priv_conf.ip_addr, this->Data->tuntap_priv_conf.netmask,
                                this->Data->tuntap_priv_conf.device_mac, this->Data->tuntap_priv_conf.mtu,
                                this->Data->tuntap_priv_conf.metric) < 0)
                    return 1;
                memcpy(&this->Data->device, &this->TunDevice, sizeof(this->TunDevice));
                traceEvent(TRACE_NORMAL, "created local tap device IP: %s, Mask: %s, MAC: %s",
                                         this->Data->tuntap_priv_conf.ip_addr,
                                         this->Data->tuntap_priv_conf.netmask,
                                         macaddr_str(mac_buf, this->Data->device.mac_addr));
                runlevel = 5;
                // no more answers required
                seek_answer = 0;
            }
            ...
            if (seek_answer) {
                FD_ZERO(&socket_mask);
                FD_SET(this->Data->sock, &socket_mask);
                wait_time.tv_sec = BOOTSTRAP_TIMEOUT;
                wait_time.tv_usec = 0;

                if (select(this->Data->sock + 1, &socket_mask, NULL, NULL, &wait_time) > 0) {
                    if (FD_ISSET(this->Data->sock, &socket_mask)) {

                        fetch_and_eventually_process_data(this->Data, this->Data->sock,
                            pktbuf, &expected, &position,
                            now);
                    }
                }
            ...
            resolve_check(this->Data->resolve_parameter, 0 /* no intermediate resolution requirement at this point */, now);
        }
        // allow a higher number of pings for first regular round of ping
        // to quicker get an inital 'supernode selection criterion overview'
        this->Data->conf.number_max_sn_pings = NUMBER_SN_PINGS_INITIAL;
        // shape supernode list; make current one the first on the list
        HASH_ITER(hh, this->Data->conf.supernodes, scan, scan_tmp) {
            if (scan == this->Data->curr_sn)
                sn_selection_criterion_good(&(scan->selection_criterion));
            else
                sn_selection_criterion_default(&(scan->selection_criterion));
        }
        sn_selection_sort(&(this->Data->conf.supernodes));
        // do not immediately ping again, allow some time
        this->Data->last_sweep = now - SWEEP_TIME + 2 * BOOTSTRAP_TIMEOUT;
        this->Data->sn_wait = 1;
        this->Data->last_register_req = 0;

#ifndef _WIN32
        if (this->Data->tuntap_priv_conf.daemon) {
            setUseSyslog(1); /* traceEvent output now goes to syslog. */
            daemonize();
        }
        ...
        if ((this->Data->tuntap_priv_conf.userid != 0) || (this->Data->tuntap_priv_conf.groupid != 0)) {
            traceEvent(TRACE_NORMAL, "dropping privileges to uid=%d, gid=%d",
                (signed int)this->Data->tuntap_priv_conf.userid, (signed int)this->Data->tuntap_priv_conf.groupid);

            /* Finished with the need for root privileges. Drop to unprivileged user. */
            if ((setgid(this->Data->tuntap_priv_conf.groupid) != 0)
                || (setuid(this->Data->tuntap_priv_conf.userid) != 0)) {
                traceEvent(TRACE_ERROR, "unable to drop privileges [%u/%s]", errno, strerror(errno));
                return 1;
            }
        }
        ...
        this->Data->keep_running = &this->_keep_running._Storage._Value;
        traceEvent(TRACE_NORMAL, "edge started");
        int rc = run_edge_loop(this->Data);
        print_edge_stats(this->Data);
        ...
        /* Cleanup */
        edge_term_conf(&this->Data->conf);
        tuntap_close(&this->Data->device);
        edge_term(this->Data);
    ...
    bool Edge::Start() {
        ...
        // this->_current.value().wait();
        return true;
    }

    void Edge::NotifyStop()
    {
...
using namespace N2N_DLL;

int Edge::StaticFunctions::NativeInvoke_main(int argc, array<String^>^ argv) {
    char** _args_native = new char* [argc];

    // Convert managed strings (C#) to native strings
    for (int i = 0; i < argc; i++) {
        auto _arg = CLR_CPP::StringHelper::GetNativeStringA(argv[i]);

        _args_native[i] = new char[_arg.length() + 1];
        strcpy(_args_native[i], _arg.c_str());
    }

    // Invoke Main
    int _r = main(argc, _args_native);

    for (int i = 0; i < argc; i++) {
        delete[] _args_native[i];
    }
    delete[] _args_native;
    return _r;
}
