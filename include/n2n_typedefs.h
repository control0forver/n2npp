...
struct n2n_udphdr;
struct port_range;
struct filter_rule_key;
struct filter_rule;
struct tuntap_dev;
struct n2n_ip_subnet;
struct n2n_sock;
struct n2n_auth;
struct n2n_common;
struct n2n_REGISTER;
...
struct n2n_REGISTER_SUPER_ACK_payload;
struct n2n_UNREGISTER_SUPER;
struct n2n_PEER_INFO;
struct n2n_QUERY_PEER;
struct peer_info;
struct host_info;
struct packet_address_proto_info;
struct filter_rule_pair_cache;
struct network_traffic_filter;
struct n2n_edge_callbacks;
struct n2n_tuntap_priv_config;
struct n2n_trans_op;
struct n2n_resolve_ip_sock;
struct n2n_resolve_parameter;
struct n2n_edge_conf;
struct n2n_edge_stats;
struct n2n_edge;
struct sn_stats;
struct node_supernode_association;
struct sn_user;
struct sn_community;
struct sn_community_regular_expression;
struct n2n_tcp_connection;
struct n2n_sn;
/* Circular definitions */
...
struct filter_rule_key {
    ...
    port_range           src_port_range;
    in_addr_t            dst_net_cidr;
    uint8_t              dst_net_bit_len;
    port_range           dst_port_range;
...
enum n2n_auth_scheme {
    n2n_auth_none =          0,
    n2n_auth_simple_id =     1,
    n2n_auth_user_password = 2
};

enum update_edge_ret_value {
...
struct n2n_REGISTER {
    ...
    n2n_sock           sock;        /**< Supernode's view of edge socket OR edge's preferred local socket */
    n2n_ip_subnet      dev_addr;    /**< IP address of the tuntap adapter. */
...
struct n2n_REGISTER_ACK {
    ...
    n2n_sock        sock;      /**< Supernode's view of edge socket (IP Addr, port) */
};
...
struct n2n_REGISTER_SUPER {
    ...
    n2n_sock           sock;        /**< Sending socket associated with edgeMac */
    n2n_ip_subnet      dev_addr;    /**< IP address of the tuntap adapter. */
    n2n_desc_t         dev_desc;    /**< Hint description correlated with the edge */
    n2n_auth           auth;        /**< Authentication scheme and tokens */
...
struct n2n_REGISTER_SUPER_ACK {
    ...
    n2n_ip_subnet      dev_addr;    /**< Assign an IP address to the tuntap adapter of edge. */
    uint16_t           lifetime;    /**< How long the registration will live */
    n2n_sock           sock;        /**< Sending sockets associated with edge */
    n2n_auth           auth;        /**< Authentication scheme and tokens */
...
struct network_traffic_filter {
    n2n_verdict (*filter_packet_from_peer)(network_traffic_filter* filter, n2n_edge *eee,
                                           const n2n_sock *peer, uint8_t *payload, uint16_t payload_size);

    n2n_verdict (*filter_packet_from_tap)(network_traffic_filter* filter, n2n_edge *eee, uint8_t *payload, uint16_t payload_size);

    filter_rule *rules;

    filter_rule_pair_cache *connections_rule_cache;
...
struct n2n_edge_callbacks {
    ...
    void (*sn_registration_updated)(n2n_edge *eee, time_t now, const n2n_sock *sn);
    ...
    n2n_verdict (*packet_from_peer)(n2n_edge *eee, const n2n_sock *peer, uint8_t *payload, uint16_t *payload_size);
    ...
    n2n_verdict (*packet_from_tap)(n2n_edge *eee, uint8_t *payload, uint16_t *payload_size);

    /* Called whenever the IP address of the TAP interface changes. */
    void (*ip_address_changed)(n2n_edge *eee, uint32_t old_ip, uint32_t new_ip);

    /* Called periodically in the main loop. */
    void (*main_loop_period)(n2n_edge *eee, time_t now);

    /* Called when a new socket to supernode is created. */
    void (*sock_opened)(n2n_edge *eee);
};
...
struct n2n_resolve_ip_sock {
    ...
    n2n_sock      sock;               /* resolved socket */
    n2n_sock      *org_sock;          /* pointer to original socket where 'sock' gets copied to from time to time */
...
struct n2n_resolve_parameter {
    n2n_resolve_ip_sock     *list;         /* pointer to list of to be resolved nodes */
...
struct n2n_edge_conf {
    ...
    n2n_transform            transop_id;             /**< The transop to use. */
    ...
    n2n_sock                 preferred_sock;         /**< propagated local sock for better p2p in LAN (-e) */
    ...
    n2n_auth                 auth;
    filter_rule              *network_traffic_filter_rules;
...
struct n2n_edge {
    ...
    n2n_trans_op                     transop;                            /**< The transop to use when encoding */
    n2n_trans_op                     transop_lzo;                        /**< The transop for LZO  compression */
#ifdef HAVE_ZSTD
    n2n_trans_op_t                   transop_zstd;                       /**< The transop for ZSTD compression */
#endif
    n2n_edge_callbacks   cb;                                             /**< API callbacks */
    ...
    n2n_sock                         multicast_peer;                     /**< Multicast peer group (for local edges) */
...
/* Timers */
    ...
    n2n_resolve_parameter            *resolve_parameter;                 /**< Pointer to name resolver's parameter block */
    uint8_t                          resolution_request;                 /**< Flag an immediate DNS resolution request */

    n2n_tuntap_priv_config           tuntap_priv_conf;                   /**< Tuntap config */

    network_traffic_filter           *network_traffic_filter;
};
...
struct sn_community {
    ...
    node_supernode_association    *assoc;                 /* list of other edges from this community and their supernodes */
    sn_user                       *allowed_users;         /* list of allowed users */
    int64_t                       number_enc_packets;     /* Number of encrypted packets handled so far, required for sorting from time to time */
    n2n_ip_subnet                 auto_ip_net;            /* Address range of auto ip address service. */
...
struct n2n_sn {
    ...
    n2n_tcp_connection                     *tcp_connections;/* list of established TCP connections */
    int                                    mgmt_sock;       /* management socket. */
    n2n_ip_subnet                          min_auto_ip_net; /* Address range of auto_ip service. */
    n2n_ip_subnet                          max_auto_ip_net; /* Address range of auto_ip service. */
#ifndef _WIN32
    uid_t                                  userid;
    gid_t                                  groupid;
#endif
