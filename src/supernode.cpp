...
#include "supernode.h"
#include "n2n.h"               // for n2n_sn, sn_community, traceEvent
...
static n2n_sn sss_node;

void close_tcp_connection(n2n_sn* sss, n2n_tcp_connection* conn);
void calculate_shared_secrets(n2n_sn* sss);
int load_allowed_sn_community(n2n_sn* sss);
int resolve_create_thread(n2n_resolve_parameter** param, struct peer_info* sn_list);

constexpr option cli_long_options_data[] = {
...
static int setOption(int optkey, char* _optarg, n2n_sn* sss) {
    ...
    case 'l': { /* supernode:port */
        ...
        socket = (n2n_sock*)calloc(1, sizeof(n2n_sock));
        ...
        if (sss->federation != NULL) {
            ...
            if (anchor_sn != NULL) {
                anchor_sn->ip_addr = (char*)calloc(1, N2N_EDGE_SN_HOST_SIZE);
                if (anchor_sn->ip_addr) {
                    strncpy(anchor_sn->ip_addr, _optarg, N2N_EDGE_SN_HOST_SIZE - 1);
                    memcpy(&(anchor_sn->sock), socket, sizeof(n2n_sock));
    ...
    case 'c': /* community file */
        sss->community_file = (char*)calloc(1, strlen(_optarg) + 1);
...
static int loadFromCLI(int argc, char* const argv[], n2n_sn* sss) {
...
static int loadFromFile(const char* path, n2n_sn* sss) {
...
static int add_federation_to_communities(n2n_sn* sss) {
...
{
    auto sn = n2n::sn::Supernode();
    sn.LoadOptionsCLI(argc, argv);

    [[maybe_unused]] auto _ = sn.Start();
    ...
    while (sn.IsRunning()) {
        printf("Supernode is pending stop, please wait...\n");
...
namespace n2n::sn
{
    int Supernode::main(int argc, char* const argv[]) { return ::main(argc, argv); }
    ...
    Supernode::Supernode()
    {
        ...
        sn_init_defaults(&this->Data);
        add_federation_to_communities(&this->Data);
    }

    void Supernode::OnOptionsLoaded()
    {
        if (this->Data.community_file)
            load_allowed_sn_community(&this->Data);

        // warn on default federation name
        if (!strcmp(this->Data.federation->community, FEDERATION_NAME)) {
            traceEvent(TRACE_WARNING, "using default federation name; FOR TESTING ONLY, usage of a custom federation name (-F) is highly recommended!");
        }

        if (this->Data.override_spoofing_protection) {
            traceEvent(TRACE_WARNING, "disabled MAC and IP address spoofing protection; FOR TESTING ONLY, usage of user-password authentication (-I, -J, -P) recommended instead!");
        }

        calculate_shared_secrets(&this->Data);
    }

    int Supernode::LoadOptions(const std::vector<std::tstring>& options)
    {
        ...
        if (loadFromCLI(_args_count, _args.data(), &this->Data) == 0)
        {
            OnOptionsLoaded();
            return 0;
        }
    ...
    int Supernode::LoadOptionsCLI(int argc, tchar* const argv[])
    {
        ...
        if (loadFromCLI(argc, _args.data(), &this->Data) == 0)
        {
            OnOptionsLoaded();
            return 0;
        }
    ...
    int Supernode::LoadOptionsFile(const std::tstring& path)
    {
        if (loadFromFile(path.c_str(), &this->Data) == 0)
        {
            OnOptionsLoaded();
            return 0;
        }
    ...
    ::n2n::IDaemon::DaemonStatus Supernode::ValidateDaemon()
    {
        typedef ::n2n::IDaemon::DaemonStatus DaemonStatus_t;

        if (!this->Data.daemon)
            return DaemonStatus_t::DaemonRunning;

#if defined(N2N_HAVE_DAEMON)
        if (getppid() == 1)
            return DaemonStatus_t::IsDaemon;

        setUseSyslog(1);

        traceEvent(TRACE_INFO, "Attempting to start daemon...");

        auto start_daemon = daemon(0, 0);
        if (start_daemon == -1)
        {
            traceEvent(TRACE_ERROR, "Failed to become daemon: %s (%d)",
                strerror(errno), errno);
            return DaemonStatus_t::DaemonKilled;
        }

        if (getppid() != 1)
        {
            traceEvent(TRACE_ERROR, "Daemonization failed, parent pid is %d", getppid());
            return DaemonStatus_t::DaemonKilled;
        }

        traceEvent(TRACE_INFO, "Daemon started successfully");
        return DaemonStatus_t::DaemonStarting;

#else
        traceEvent(TRACE_WARNING, "Daemon support not compiled in");
        return DaemonStatus_t::NotSupported;
    ...
    int Supernode::Run()
    {
        ...
        // Start Daemon
        auto validate_daemon = ValidateDaemon();
        // TODO: ValidateDaemon return value
        //if (validate_daemon != ::n2n::IDaemon::DaemonStatus::DaemonRunning)
        //{
        //    traceEvent(TRACE_WARNING, "daemon could't be started: Daemon is not running.");
        //}

        // Setup Main Socket
        if (OpenMainSocket(&this->Data) == -2)
        {
            traceEvent(TRACE_ERROR, "Failed to initialize main communication socket. Aborting.");
            return -1;
        }
        // Setup Management Socket
        if (OpenManagementSocket(&this->Data) == -2)
        {
            traceEvent(TRACE_WARNING, "Management socket initialization failed. Continuing without management interface.");
        }

        // Start Auxiliary Socket
        auto start_auxiliary_socket = StartAuxiliarySocket(&this->Data);
        if (start_auxiliary_socket != 0)
        {
            if (start_auxiliary_socket == 2)
                traceEvent(TRACE_INFO, "Auxiliary socket could not be established: TCP acceleration socket disabled on Windows platform. Using standard UDP mode.");
            
            if (start_auxiliary_socket == 1)
                traceEvent(TRACE_WARNING, "Auxiliary socket could not be established: Failed to establish TCP acceleration socket. Falling back to standard UDP performance.");
        }

        // Setup federation with main UDP socket
        if (AssignMainSocket(&this->Data) != 0)
        {
            traceEvent (TRACE_ERROR,  "Failed to initialize federation. Aborting.");
            return -2;
        }
        ...
        return run_sn_loop(&this->Data);
    }

    bool Supernode::Start() {
        if (IsRunning())
            return false;
    ...
    void Supernode::NotifyStop()
    {
        this->_keep_running = false;
    }

    
    #pragma region sn proc

    int Supernode::OpenMainSocket(n2n_sn* conf)
    {
        auto _sock = open_socket(conf->lport, conf->bind_address, 0 /* UDP */);
        if (-1 == _sock)
        {
            traceEvent(TRACE_ERROR, "failed to open main socket. %s", strerror(errno));
            return -2;
        ...
        traceEvent(TRACE_NORMAL, "supernode is listening on UDP %u (main)", conf->lport);
        return 0;
    }

    int Supernode::OpenManagementSocket(n2n_sn* conf)
    {
        auto _mgmt_sock = open_socket(conf->mport, INADDR_LOOPBACK, 0 /* UDP */);
        if (-1 == _mgmt_sock) {
            traceEvent(TRACE_ERROR, "failed to open management socket, %s", strerror(errno));
            return -2;
        }

        conf->mgmt_sock = _mgmt_sock;

        traceEvent(TRACE_NORMAL, "supernode is listening on UDP %u (management)", conf->mport);
        return 0;
    }

    int Supernode::StartAuxiliarySocket(n2n_sn* conf)
    {
#if defined(N2N_HAVENO_TCP_WIN)
        return 2;
#elif !defined(N2N_HAVE_TCP)
        return 1;
#endif

        conf->tcp_sock = open_socket(conf->lport, conf->bind_address, 1 /* TCP */);
        if (-1 == conf->tcp_sock) {
            traceEvent(TRACE_ERROR, "failed to open auxiliary TCP socket, %s", strerror(errno));
            return -2;
        }
        else {
            traceEvent(TRACE_NORMAL, "supernode opened TCP %u (aux)", conf->lport);
        }

        if (-1 == listen(conf->tcp_sock, N2N_TCP_BACKLOG_QUEUE_SIZE)) {
            traceEvent(TRACE_ERROR, "failed to listen on auxiliary TCP socket, %s", strerror(errno));
            return -2;
        }
        else {
            traceEvent(TRACE_NORMAL, "supernode is listening on TCP %u (aux)", conf->lport);
        }

        return 0;
    }

    int Supernode::AssignMainSocket(n2n_sn* conf)
    {
        peer_info *_scan, *_tmp;
        HASH_ITER(hh, conf->federation->edges, _scan, _tmp)
            _scan->socket_fd = conf->sock;
...
int Supernode::StaticFunctions::NativeInvoke_main(int argc, array<String^>^ argv) {
