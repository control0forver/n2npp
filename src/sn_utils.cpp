...
#include "n2n.h"                // for sn_community, n2n_sn, peer_info
...
static ssize_t sendto_peer (n2n_sn *sss,
                            const struct peer_info *peer,
                            const uint8_t *pktbuf,
                            size_t pktsize);

static uint16_t reg_lifetime (n2n_sn *sss);

static int update_edge (n2n_sn *sss,
                        const n2n_common* cmn,
                        const n2n_REGISTER_SUPER* reg,
                        struct sn_community *comm,
                        const n2n_sock *sender_sock,
                        const SOCKET socket_fd,
                        n2n_auth *answer_auth,
                        int skip_add,
                        time_t now);

static int re_register_and_purge_supernodes (n2n_sn *sss,
                                             struct sn_community *comm,
                                             time_t *p_last_re_reg_and_purge,
                                             time_t now,
                                             uint8_t forced);

static int purge_expired_communities (n2n_sn *sss,
                                      time_t* p_last_purge,
                                      time_t now);

static int sort_communities (n2n_sn *sss,
                             time_t* p_last_sort,
                             time_t now);

int process_mgmt (n2n_sn *sss,
                  const struct sockaddr *sender_sock, socklen_t sock_size,
                  char *mgmt_buf,
                  size_t mgmt_size,
                  time_t now);

static int process_udp (n2n_sn *sss,
                        const struct sockaddr *sender_sock, socklen_t sock_size,
                        const SOCKET socket_fd,
                        uint8_t *udp_buf,
                        size_t udp_size,
...
void calculate_dynamic_keys (n2n_sn *sss) {
...
void send_re_register_super (n2n_sn *sss) {
...
int load_allowed_sn_community (n2n_sn *sss) {
    ...
    struct peer_info *edge, *tmp_edge;
    node_supernode_association *assoc, *tmp_assoc;
    n2n_tcp_connection *conn;
    time_t any_time = 0;
        ...
        if(line[0] == N2N_USER_KEY_LINE_STARTER) { /* special first character */
            if(sscanf(line, format, username, ascii_public_key) == 2) { /* correct format */
                if(last_added_comm) { /* is there a valid community to add users to */
                    user = (sn_user*)calloc(1, sizeof(sn_user));
                    if(user) {
                        // username
 ...
 */
static ssize_t sendto_fd (n2n_sn *sss,
                          SOCKET socket_fd,
    ...
    sent = sendto(socket_fd, (const char *)pktbuf, pktsize, 0 /* flags */,
 ...
 *    @return -1 on error otherwise number of bytes sent
 */
static ssize_t sendto_sock(n2n_sn *sss,
                           SOCKET socket_fd,
                           const struct sockaddr *socket,
                           const uint8_t *pktbuf,
                           size_t pktsize) {
    ...
    if((socket_fd >= 0) && (socket_fd != sss->sock)) {
        ...
        value = 1;
#ifdef LINUX
        setsockopt(socket_fd, IPPROTO_TCP, TCP_CORK, (const char*)&value, sizeof(value));
#endif
...
/** Send a datagram to a peer whose destination socket is embodied in its sock field of type n2n_sock.
 ...
 *    the supernode.
 */
static int try_broadcast (n2n_sn * sss,
...
static int try_forward (n2n_sn * sss,
                        const struct sn_community *comm,
                        const n2n_common * cmn,
    ...
    node_supernode_association   *assoc;
    macstr_t                       mac_buf;
    n2n_sock_str_t                 sockbuf;
...
int sn_init_defaults (n2n_sn *sss) {
 ...
 *    it. */
void sn_term (n2n_sn *sss) {

    struct sn_community *community, *tmp;
    struct sn_community_regular_expression *re, *tmp_re;
    n2n_tcp_connection *conn, *tmp_conn;
    node_supernode_association *assoc, *tmp_assoc;
...
void update_node_supernode_association (struct sn_community *comm,
    ...
    if(!assoc) {
        ...
        assoc = (node_supernode_association*)calloc(1, sizeof(node_supernode_association));
        if(assoc) {
 ...
 */
static int auth_edge (const n2n_auth *present, const n2n_auth *presented, n2n_auth *answer, struct sn_community *community) {
    ...
    if(present->scheme == n2n_auth_none) {
        ...
        if(answer)
            memset(answer, 0, sizeof(n2n_auth));
        // 0 == (always) successful
        return 0;
    }
    ...
    if((present->scheme == n2n_auth_simple_id) && (presented->scheme == n2n_auth_simple_id)) {
        ...
        return (memcmp(present, presented, sizeof(n2n_auth)));
    }
    ...
    if((present->scheme == n2n_auth_user_password) && (presented->scheme == n2n_auth_user_password)) {
        ...
        if(user) {
            if(answer) {
                memcpy(answer, presented, sizeof(n2n_auth));
...
static int get_local_auth (n2n_sn *sss, n2n_auth *auth) {

    // n2n_auth_simple_id scheme
    memcpy(auth, &(sss->auth), sizeof(n2n_auth));
...
static int handle_remote_auth (n2n_sn *sss, const n2n_auth *remote_auth,
                                              n2n_auth *answer_auth,
    ...
    /* Not known */
        if(handle_remote_auth(sss, &(reg->auth), answer_auth, comm) == 0) {
            if(skip_add == SN_ADD) {
                ...
                memcpy(&(scan->sock), sender_sock, sizeof(n2n_sock));
                ...
                if(cmn->flags & N2N_FLAGS_SOCKET)
                    memcpy(&scan->preferred_sock, &reg->sock, sizeof(n2n_sock));
                else
                    scan->preferred_sock.family = AF_INVALID;

                // store the submitted auth token
                memcpy(&(scan->auth), &(reg->auth), sizeof(n2n_auth));
...
static int ip_addr_available (struct sn_community *comm, n2n_ip_subnet *ip_addr) {
...
static int assign_one_ip_addr (struct sn_community *comm, n2n_desc_t dev_desc, n2n_ip_subnet *ip_addr) {
...
int subnet_available (n2n_sn *sss,
                      struct sn_community *comm,
                      uint32_t net_id,
                      uint32_t mask) {
...
int assign_one_ip_subnet (n2n_sn *sss,
                          struct sn_community *comm) {
...
static int re_register_and_purge_supernodes (n2n_sn *sss, struct sn_community *comm, time_t *p_last_re_reg_and_purge, time_t now, uint8_t forced) {
...
static int process_udp (n2n_sn * sss,
    ...
    switch(msg_type) {
        case MSG_TYPE_PACKET: {
            ...
            if(!from_supernode) {
                memcpy(&cmn2, &cmn, sizeof(n2n_common));
        ...
        case MSG_TYPE_REGISTER_SUPER: {
            ...
            uint8_t                                match = 0;
            int                                    match_length = 0;
            n2n_ip_subnet                          ipaddr;
            int                                    num = 0;
            int                                    skip;
            int                                    ret_value;
            sn_user                                *user = NULL;

            memset(&ack, 0, sizeof(n2n_REGISTER_SUPER_ACK));
            ...
            */
            ...
            if(comm->is_federation != IS_FEDERATION) { /* alternatively, do not send zero tap ip address in federation REGISTER_SUPER */
                if((reg.dev_addr.net_addr == 0) || (reg.dev_addr.net_addr == 0xFFFFFFFF) || (reg.dev_addr.net_bitlen == 0) ||
                   ((reg.dev_addr.net_addr & 0xFFFF0000) == 0xA9FE0000 /* 169.254.0.0 */)) {
                    memset(&ipaddr, 0, sizeof(n2n_ip_subnet));
            ...
            payload = (n2n_REGISTER_SUPER_ACK_payload*)payload_buf;
            HASH_ITER(hh, sss->federation->edges, peer, tmp_peer) {
                ...
                if(memcmp(&(peer->sock), &(ack.sock), sizeof(n2n_sock)) == 0) continue; /* a supernode doesn't add itself to the payload */
        ...
        case MSG_TYPE_UNREGISTER_SUPER: {
            ...
            memset(&unreg, 0, sizeof(n2n_UNREGISTER_SUPER));
        ...
        case MSG_TYPE_REGISTER_SUPER_ACK: {
            ...
            if(ack.cookie == scan->last_cookie) {
                ...
                payload = (n2n_REGISTER_SUPER_ACK_payload *)dec_tmpbuf;
    ...
    while(*sss->keep_running) {
        ...
        if(rc > 0) {
            ...
            if(FD_ISSET(sss->sock, &socket_mask)) {
                ...
                bread = recvfrom(sss->sock, (char *)pktbuf, N2N_SN_PKTBUF_SIZE, 0 /*flags*/,
                                 sender_sock, &ss_size);
            ...
            HASH_ITER(hh, sss->tcp_connections, conn, tmp_conn) {
                ...
                if(FD_ISSET(conn->socket_fd, &socket_mask)) {
                    ...
                    if(bread <= 0) {
                        traceEvent(TRACE_INFO, "closing tcp connection to [%s]", sock_to_cstr(sockbuf, (n2n_sock*)sender_sock));
            ...
            if(FD_ISSET(sss->tcp_sock, &socket_mask)) {
                ...
                if((HASH_COUNT(sss->tcp_connections) + 4) < FD_SETSIZE) {
                    ...
                    if(tmp_sock >= 0) {
                        conn = (n2n_tcp_connection*)calloc(1, sizeof(n2n_tcp_connection));
                        if(conn) {
                            ...
                            traceEvent(TRACE_INFO, "accepted incoming TCP connection from [%s]",
                                                   sock_to_cstr(sockbuf, (n2n_sock*)sender_sock));
                        }
                    }
                } else {
            ...
            if(FD_ISSET(sss->mgmt_sock, &socket_mask)) {
                ...
                bread = recvfrom(sss->mgmt_sock, (char *)pktbuf, N2N_SN_PKTBUF_SIZE, 0 /*flags*/,
                                 sender_sock, &ss_size);
