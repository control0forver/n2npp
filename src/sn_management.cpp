...
#include "n2n.h"         // for n2n_sn, sn_community, peer_info, N2N_SN_PK...
#include "n2n_define.h"    // for N2N_SN_PKTBUF_SIZE, UNPURGEABLE
#include "n2n_typedefs.h"  // for n2n_sn, sn_community, peer_info, sn_stats_t
...
int load_allowed_sn_community (n2n_sn *sss); /* defined in sn_utils.c */

static void mgmt_reload_communities (mgmt_req_t *req, strbuf *buf) {
...
static const mgmt_handler_t mgmt_handlers[] = {
    ...
    { .flags = FLAG_WROK, .cmd = "reload_communities", .help = "Reloads communities and user's public keys", .func = mgmt_reload_communities},
...
static int sendto_mgmt (n2n_sn *sss,
                        const struct sockaddr *sender_sock, socklen_t sock_size,
                        const uint8_t *mgmt_buf,
                        size_t mgmt_size) {

    ssize_t r = sendto(sss->mgmt_sock, (const char *)mgmt_buf, mgmt_size, 0 /*flags*/,
