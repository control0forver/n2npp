...
#include "n2n.h"                     // for n2n_edge, peer_info, n2n_edge_...
...
void resolve_cancel_thread (n2n_resolve_parameter *param);

static const char * supernode_ip (const n2n_edge * eee);
static void send_register (n2n_edge *eee, const n2n_sock *remote_peer, const n2n_mac_t peer_mac, n2n_cookie_t cookie);

static void check_peer_registration_needed (n2n_edge *eee,
                                            uint8_t from_supernode,
                                            uint8_t via_multicast,
                                            const n2n_mac_t mac,
                                            const n2n_cookie_t cookie,
                                            const n2n_ip_subnet *dev_addr,
                                            const n2n_desc_t *dev_desc,
                                            const n2n_sock *peer);

static int edge_init_sockets (n2n_edge *eee);

static void check_known_peer_sock_change (n2n_edge *eee,
                                          uint8_t from_supernode,
                                          uint8_t via_multicast,
                                          const n2n_mac_t mac,
                                          const n2n_ip_subnet *dev_addr,
...
int edge_verify_conf (const n2n_edge_conf *conf) {
...
void edge_set_callbacks (n2n_edge *eee, const n2n_edge_callbacks *callbacks) {

    memcpy(&eee->cb, callbacks, sizeof(n2n_edge_callbacks));
}

/* ************************************** */

void edge_set_userdata (n2n_edge *eee, void *user_data) {
...
void* edge_get_userdata (n2n_edge *eee) {
...
int edge_get_n2n_socket (n2n_edge *eee) {
...
int edge_get_management_socket (n2n_edge *eee) {
...
static int is_ethMulticast (const void * buf, size_t bufsize) {
    ...
    if(bufsize >= sizeof(ether_hdr)) {
        /* copy to aligned memory */
        ether_hdr eh;
        memcpy(&eh, buf, sizeof(ether_hdr));
...
void reset_sup_attempts (n2n_edge *eee) {
...
static int detect_local_ip_address (n2n_sock* out_sock, const n2n_edge* eee) {
...
int supernode_connect (n2n_edge *eee) {
    ...
    if(eee->sock < 0) {
        ...
        memset(&local_sock, 0, sizeof(n2n_sock));
        if(detect_local_ip_address(&local_sock, eee) == 0) {
            ...
            if(eee->conf.preferred_sock_auto) {
                ...
                memcpy(&eee->conf.preferred_sock, &local_sock, sizeof(n2n_sock));
...
void supernode_disconnect (n2n_edge *eee) {
    if(!eee) {
        return;
    }
    if(eee->sock >= 0) {
 ...
 */
n2n_edge* edge_init (const n2n_edge_conf *conf, int *rv) {

    n2n_transform transop_id = conf->transop_id;
    n2n_edge *eee = (n2n_edge*)calloc(1, sizeof(n2n_edge));
    int rc = -1, i = 0;
 ...
 */
static int is_valid_peer_sock (const n2n_sock *sock) {
 ...
 */
static int find_peer_time_stamp_and_verify (n2n_edge * eee,
                                            peer_info *sn, const n2n_mac_t mac,
 ...
 */
static void register_with_local_peers (n2n_edge * eee) {
#ifndef SKIP_MULTICAST_PEERS_DISCOVERY
...
static struct peer_info* find_peer_by_sock (const n2n_sock *sock, struct peer_info *peer_list) {

    struct peer_info *scan, *tmp, *ret = NULL;

    HASH_ITER(hh, peer_list, scan, tmp) {
        if(memcmp(&(scan->sock), sock, sizeof(n2n_sock)) == 0) {
            ret = scan;
            break;
        }
    }
 ...
 */
static void register_with_new_peer (n2n_edge *eee,
                                    uint8_t from_supernode,
                                    uint8_t via_multicast,
                                    const n2n_mac_t mac,
                                    const n2n_ip_subnet *dev_addr,
    ...
    if(scan == NULL) {
        scan = (peer_info*)calloc(1, sizeof(struct peer_info));
    ...
    if(dev_addr != NULL) {
        memcpy(&(scan->dev_addr), dev_addr, sizeof(n2n_ip_subnet));
    }
 ...
 */
static void peer_set_p2p_confirmed (n2n_edge * eee,
                                    const n2n_mac_t mac,
                                    const n2n_cookie_t cookie,
                                    const n2n_sock * peer,
                                    time_t now) {
...
static int get_local_auth (n2n_edge *eee, n2n_auth *auth) {

    switch(eee->conf.auth.scheme) {
        case n2n_auth_simple_id:
            memcpy(auth, &(eee->conf.auth), sizeof(n2n_auth));
            break;
...
static int handle_remote_auth (n2n_edge *eee, struct peer_info *peer, const n2n_auth *remote_auth) {
...
int is_empty_ip_address (const n2n_sock * sock) {
 ...
 */
static int check_sock_ready (n2n_edge *eee) {
...
static ssize_t sendto_fd (n2n_edge *eee, const void *buf,
                          size_t len, struct sockaddr_in *dest,
                          const n2n_sock * n2ndest) {
    ...
    sent = sendto(eee->sock, (const char*)buf, len, 0 /*flags*/,
...
/** Send a datagram to a socket defined by a n2n_sock */
static void sendto_sock (n2n_edge *eee, const void * buf,
                            size_t len, const n2n_sock * dest) {
...
static void check_join_multicast_group (n2n_edge *eee) {
...
void send_query_peer (n2n_edge * eee,
                      const n2n_mac_t dst_mac) {

    uint8_t pktbuf[N2N_PKT_BUF_SIZE];
    size_t idx;
    n2n_common cmn = {0};
    n2n_QUERY_PEER query = {0};
...
void send_register_super (n2n_edge *eee) {
    ...
    if(eee->conf.preferred_sock.family == (uint8_t)AF_INVALID) {
        ...
        memcpy(&(reg.sock), &(eee->conf.preferred_sock), sizeof(n2n_sock));
    }
    memcpy(cmn.community, eee->conf.community_name, N2N_COMMUNITY_SIZE);
...
static void send_unregister_super (n2n_edge *eee) {
...
static int sort_supernodes (n2n_edge *eee, time_t now) {
...
static void send_register (n2n_edge * eee,
                           const n2n_sock * remote_peer,
...
static void send_register_ack (n2n_edge * eee,
                               const n2n_sock * remote_peer,
...
static int build_gratuitous_arp (n2n_edge * eee, char *buffer, uint16_t buffer_len) {
 ...
 *    broadcasts. */
static void send_grat_arps (n2n_edge * eee) {
 ...
 */
void update_supernode_reg (n2n_edge * eee, time_t now) {
 ...
 *    encrypted. */
static int handle_PACKET (n2n_edge * eee,
                          const uint8_t from_supernode,
                          const n2n_PACKET * pkt,
                          const n2n_sock * orig_sender,
                          uint8_t * payload,
                          size_t psize) {
    ...
    /* Handle transform. */
    {
        ...
        size_t eth_size;
        n2n_transform rx_transop_id;
        uint8_t rx_compression_id;

        rx_transop_id = (n2n_transform)pkt->transform;
        ...
        if(rx_transop_id == eee->conf.transop_id) {
            ...
            if((eee->conf.allow_routing) && (!is_multi_broadcast(eh->shost))) {
                ...
                if(host == NULL) {
                    struct host_info *host = (host_info*)calloc(1, sizeof(struct host_info));
...
static int check_query_peer_info (n2n_edge *eee, time_t now, n2n_mac_t mac) {
...
static int find_peer_destination (n2n_edge * eee,
                                  n2n_mac_t mac_address,
                                  n2n_sock * destination) {
    ...
    if(scan && (scan->last_seen > 0)) {
        if((now - scan->last_p2p) >= (scan->timeout / 2)) {
            ...
            memcpy(destination, &scan->sock, sizeof(n2n_sock));
            retval = 1;
        }
    }
 ...
 *    address. */
static int send_packet (n2n_edge * eee,
                        n2n_mac_t dstMac,
...
void edge_send_packet2net (n2n_edge * eee,
    ...
    uint8_t pktbuf[N2N_PKT_BUF_SIZE];
    size_t idx = 0;
    n2n_transform tx_transop_idx = eee->transop.transform_id;
    ether_hdr eh;

    /* tap_pkt is not aligned so we have to copy to aligned memory */
    memcpy(&eh, tap_pkt, sizeof(ether_hdr));
 ...
 */
void edge_read_from_tap (n2n_edge * eee) {
...
void process_udp (n2n_edge *eee, const struct sockaddr *sender_sock, const SOCKET in_sock,
                 uint8_t *udp_buf, size_t udp_size, time_t now) {

    n2n_common          cmn; /* common fields in the packet header */
    n2n_sock_str_t        sockbuf1;
    n2n_sock_str_t        sockbuf2; /* don't clobber sockbuf1 if writing two addresses to trace */
    macstr_t              mac_buf1;
    macstr_t              mac_buf2;
    ...
    memset(&sender, 0, sizeof(n2n_sock));
    ...
    if(0 == memcmp(cmn.community, eee->conf.community_name, N2N_COMMUNITY_SIZE)) {
        switch(msg_type) {
            ...
            case MSG_TYPE_REGISTER_SUPER_ACK: {
                in_addr_t net;
                char * ip_str = NULL;
                n2n_REGISTER_SUPER_ACK ra;
                uint8_t tmpbuf[REG_SUPER_ACK_PAYLOAD_SPACE];
                char ip_tmp[N2N_EDGE_SN_HOST_SIZE];
                n2n_REGISTER_SUPER_ACK_payload *payload;
                n2n_sock payload_sock;
                int i;
                int skip_add;
                ...
                memset(&ra, 0, sizeof(n2n_REGISTER_SUPER_ACK));
                ...
                payload = (n2n_REGISTER_SUPER_ACK_payload*)tmpbuf;
                ...
                for(i = 0; i < ra.num_sn; i++) {
                    ...
                    if(skip_add == SN_ADD_ADDED) {
                        sn->ip_addr = (char*)calloc(1, N2N_EDGE_SN_HOST_SIZE);
                        if(sn->ip_addr != NULL) {
            ...
            case MSG_TYPE_REGISTER_SUPER_NAK: {
                ...
                memset(&nak, 0, sizeof(n2n_REGISTER_SUPER_NAK));
...
int fetch_and_eventually_process_data (n2n_edge *eee, SOCKET sock,
    ...
    || (sock == eee->udp_multicast_sock)
        ...
        // udp
        bread = recvfrom(sock, (char *)pktbuf, N2N_PKT_BUF_SIZE, 0 /*flags*/,
                         sender_sock, &ss_size);
        ...
        // tcp
        bread = recvfrom(sock,
                         (char *)(pktbuf + *position), *expected - *position, 0 /*flags*/,
                        sender_sock, &ss_size);
        if((bread <= 0) && (errno)) {
            traceEvent(TRACE_ERROR, "recvfrom() failed %d errno %d (%s)", bread, errno, strerror(errno));
#ifdef _WIN32
...
void print_edge_stats (const n2n_edge *eee) {
...
int run_edge_loop (n2n_edge *eee) {
...
void edge_init_conf_defaults (n2n_edge_conf *conf) {
    ...
    tmp_string = (char*)calloc(1, strlen(N2N_MGMT_PASSWORD) + 1);
    if(tmp_string) {
...
void edge_term_conf (n2n_edge_conf *conf) {

    if(conf->encrypt_key) free(conf->encrypt_key);

    if(conf->network_traffic_filter_rules) {
        filter_rule *el = 0, *tmp = 0;
...
const n2n_edge_conf* edge_get_conf (const n2n_edge *eee) {
...
int edge_conf_add_supernode (n2n_edge_conf *conf, const char *ip_and_port) {
    ...
    sock = (n2n_sock*)calloc(1,sizeof(n2n_sock));
    ...
    if(sn != NULL) {
        ...
        if(sn->ip_addr != NULL) {
            ...
            memcpy(&(sn->sock), sock, sizeof(n2n_sock));
