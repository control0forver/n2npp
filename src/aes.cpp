...
#include "aes.h"     // for AES_BLOCK_SIZE, aes_context, AES128_KEY_BYTES
...
int aes_cbc_encrypt (unsigned char *out, const unsigned char *in, size_t in_len,
                     const unsigned char *iv, aes_context *ctx) {
...
int aes_ecb_decrypt (unsigned char *out, const unsigned char *in, aes_context *ctx) {
...
int aes_init (const unsigned char *key, size_t key_size, aes_context **ctx) {

    // allocate context...
    *ctx = (aes_context*) calloc(1, sizeof(aes_context));
    if(!(*ctx))
        return -1;
...
// key setup
static int aes_internal_key_setup (aes_context *ctx, const uint8_t *key, int key_bits) {
...
static void aes_internal_encrypt (aes_context *ctx, const uint8_t pt[16], uint8_t ct[16]) {
...
static void aes_internal_decrypt (aes_context *ctx, const uint8_t ct[16], uint8_t pt[16]) {
...
// not used
int aes_ecb_encrypt (unsigned char *out, const unsigned char *in, aes_context *ctx) {
...
int aes_deinit (aes_context *ctx) {
